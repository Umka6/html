...
bot = telebot.TeleBot('1385808594:AAF0pVNLQiJvfC69DCdXlGTgO0pRarSLIpQ')
class User:
    def __init__(self, name, menu, basket):
        self.name = name
        self.menu = menu
        self.basket = basketqrcode
...
def requestdb(ID, user):
    ...
    conn = mysql.connector.connect(host='localhost', database='TG', user='root', password='root')
    c = conn.cursor()
...
def echo_all(message):
    ...
    if message.text == "Меню":
        ...
        requestdb(message.chat.id, message.chat.fetchoneirst_name)
